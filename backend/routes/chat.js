...
// });
router.post('/analyze2', auth, async (req, res) => {
    const { message, data, sheets, activeSheetId } = req.body;
    console.log("Received data:", {
        messageLength: message?.length || 0,
        dataLength: data?.length || 0,
        sheetsCount: sheets ? Object.keys(sheets).length : 0,
...
If the user mentions creating a chart in a specific sheet using data from another sheet, make sure to set the sourceSheetId and targetSheetId appropriately. If not specified, assume both are the active sheet.`;
        ...
        let messageContent = `Here is my data: ${JSON.stringify(data)}\n\n`;
        
        // Add available sheets information if provided
        if (sheets) {
            messageContent += `I have multiple sheets:\n`;
            Object.entries(sheets).forEach(([sheetId, sheetData]) => {
                ...
                messageContent += `- ${sheetName}${isActive} (ID: ${sheetId})\n`;
                
                // Add a sample of each sheet's data
        ...
        messageContent += `\nActive sheet ID: ${activeSheetId || 'none'}\n\nMy question: ${message}`;
